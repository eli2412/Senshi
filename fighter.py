...
class Fighter():
    def __init__(self, player, x ,y, flip, data, sprite_sheet, animation_steps, sound):
        ...
        self.action = 0 #0#idle #1:run ect
        self.frame_index = 0
    ...
    def move(self, window_width, window_height, surface, target, round_over):
        ...
        self.running = False
        self.attack_type = 0
        #movement of keys pressed
        keys = pygame.key.get_pressed()

        #can perform if not attack
        ...
        #jumping
                if keys[pygame.K_UP] and self.jump == False:
                    ...
                    self.jump = True
        #attack
                if keys[pygame.K_o] or keys[pygame.K_p]:
                    self.attack(target)
            #attack type
                if keys[pygame.K_o]:
                    self.attack_type = 1
    ...
    def update(self):
        #check what action player if performing
        if self.health <= 0:
            self.health = 0
            self.alive = False
        ...
        if self.frame_index >= len(self.animation_list[self.action]):
            ...
            #check if attack was executed
                ...
                if self.action == 5:
                    self.hit = False
                #if player was in middle of attack, then attack is stopped
                    self.attacking = False
                    self.attack_cooldown = 20
    ...
    def update_action(self, new_action):
        ...
        if new_action != self.action:
            ...
            #uopdate animation setting
            self.frame_index = 0
            self.update_time = pygame.time.get_ticks()

    def draw(self, surface):
        img = pygame.transform.flip(self.image, self.flip, False)
        # pygame.draw.rect(surface, (255, 0, 0,), self.rect) Rectangle 
